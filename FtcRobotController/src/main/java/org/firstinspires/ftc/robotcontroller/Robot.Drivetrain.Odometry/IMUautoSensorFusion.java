...
@Autonomous(name = "autoSensorFusion", group = "Odometric Corrected Drive")
public class IMUautoSensorFusion extends LinearOpMode
{
    ...
    public void errorCorrectedTurn(double power, double angle)
    {
        ...
        double o = (-getXAngle()+globalPositionUpdate.returnOrientation())/2;

        double t;
 
        do
        {
            t = (-getXAngle()+globalPositionUpdate.returnOrientation())/2;
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        sleep(50);    
            errorCorrectedTurn(0.35, 90);
        sleep(50);    
            errorCorrectedDriveXfb(0.65, 19.5);
            sleep(100);
            errorCorrectedTurn(-0.35, -90.0);
            ...
            errorCorrectedTurn(-0.35, -180.0);
        sleep(50);    
            errorCorrectedDriveYfb(0.65, -7.0);
...
//that I may or may not have fixed - experiment if needed
            ...
            //if 1 ring
            else if (potentiometerReading >= 2.2 && potentiometerReading <= 2.529)
            {
...
//that I may or may not have fixed - experiment if needed
 ...
 //turn a bit right to line up with wobble drop zone and drive backwards
            ...
            //if 0 rings
            else if (potentiometerReading >= 2.53)
            {
//if any changes are necessary here the drive functions work as follows
            //the drive x,y and fb/lr should stay the same and the negatives I think are
            //all right and I wouldn’t change them as they have very strange behavior
...
//that I may or may not have fixed - experiment if needed
 ...
 //turn a bit right to line up with wobble drop zone and drive backwards
                //CAN DELETE OR MODIFY TURN IF NEEDED    
                errorCorrectedTurn(-0.35, -15.0);
                sleep(50);    
                errorCorrectedDriveYfb(-0.65, 17.0);
